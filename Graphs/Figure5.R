...
setwd('C:/Users/User/TCDUD.onmicrosoft.com/Claire Gillan - Gillan Lab Resources/Projects/Transdiagnostic_Twitter/')
...
sds_betas <- merge(sds,betas_sf,by="LIWC")
oci_betas <- merge(oci,betas_sf,by="LIWC")
ssms_betas <- merge(ssms,betas_sf,by="LIWC")
bis_betas <- merge(bis,betas_sf,by="LIWC")
lsas_betas <- merge(lsas,betas_sf,by="LIWC")
eat_betas <- merge(eat,betas_sf,by="LIWC")
audit_betas <- merge(audit,betas_sf,by="LIWC")
aes_betas <- merge(aes,betas_sf,by="LIWC")
stai_betas <- merge(stai,betas_sf,by="LIWC")
...
top_20 <- as.data.frame(table(c(paste(sds_betas$LIWC,sds_betas$SDS),paste(oci_betas$LIWC,oci_betas$OCI),
                                paste(ssms_betas$LIWC,ssms_betas$SSMS),paste(bis_betas$LIWC,bis_betas$BIS),
                                paste(lsas_betas$LIWC,lsas_betas$LSAS),paste(eat_betas$LIWC,eat_betas$EAT),
                                paste(audit_betas$LIWC,audit_betas$AUDIT),paste(aes_betas$LIWC,aes_betas$AES),
                                paste(stai_betas$LIWC,stai_betas$STAI))))

colnames(top_20) <- c("LIWC","Freq")
top_20$LIWC <- tolower(top_20$LIWC)
top_20$LIWC[which(top_20$LIWC == "wc")] <- "word count"

sds <- merge(sds,betas_sf[,c("LIWC","SDS")],by="LIWC")
sds$LIWC <- paste(sds$LIWC,sds$SDS)
sds <- sds[,-11]
sds <- merge(sds,top_20,by="LIWC")
sds$Freq <- (as.factor(sds$Freq))
sds$Freq <- factor(sds$Freq, levels=rev(levels(sds$Freq)))

oci <- merge(oci,betas_sf[,c("LIWC","OCI")],by="LIWC")
oci$LIWC <- paste(oci$LIWC,oci$OCI)
oci <- oci[,-11]
oci <- merge(oci,top_20,by="LIWC")
oci$Freq <- (as.factor(oci$Freq))
oci$Freq <- factor(oci$Freq, levels=rev(levels(oci$Freq)))

ssms <- merge(ssms,betas_sf[,c("LIWC","SSMS")],by="LIWC")
ssms$LIWC <- paste(ssms$LIWC,ssms$SSMS)
ssms <- ssms[,-11]
ssms <- merge(ssms,top_20,by="LIWC")
ssms$Freq <- (as.factor(ssms$Freq))
ssms$Freq <- factor(ssms$Freq, levels=rev(levels(ssms$Freq)))

bis <- merge(bis,betas_sf[,c("LIWC","BIS")],by="LIWC")
bis$LIWC <- paste(bis$LIWC,bis$BIS)
bis <- sds[,-11]
bis <- merge(bis,top_20,by="LIWC")
bis$Freq <- (as.factor(bis$Freq))
bis$Freq <- factor(bis$Freq, levels=rev(levels(bis$Freq)))

lsas <- merge(lsas,betas_sf[,c("LIWC","LSAS")],by="LIWC")
lsas$LIWC <- paste(lsas$LIWC,lsas$LSAS)
lsas <- lsas[,-11]
lsas <- merge(lsas,top_20,by="LIWC")
lsas$Freq <- (as.factor(lsas$Freq))
lsas$Freq <- factor(lsas$Freq, levels=rev(levels(lsas$Freq)))

eat <- merge(eat,betas_sf[,c("LIWC","EAT")],by="LIWC")
eat$LIWC <- paste(eat$LIWC,eat$EAT)
eat <- eat[,-11]
eat <- merge(eat,top_20,by="LIWC")
eat$Freq <- (as.factor(eat$Freq))
eat$Freq <- factor(eat$Freq, levels=rev(levels(eat$Freq)))

audit <- merge(audit,betas_sf[,c("LIWC","AUDIT")],by="LIWC")
audit$LIWC <- paste(audit$LIWC,audit$AUDIT)
audit <- audit[,-11]
audit <- merge(audit,top_20,by="LIWC")
audit$Freq <- (as.factor(audit$Freq))
audit$Freq <- factor(audit$Freq, levels=rev(levels(audit$Freq)))

aes <- merge(aes,betas_sf[,c("LIWC","AES")],by="LIWC")
aes$LIWC <- paste(aes$LIWC,aes$AES)
aes <- aes[,-11]
aes <- merge(aes,top_20,by="LIWC")
aes$Freq <- (as.factor(aes$Freq))
aes$Freq <- factor(aes$Freq, levels=rev(levels(aes$Freq)))

stai <- merge(stai,betas_sf[,c("LIWC","STAI")],by="LIWC")
stai$LIWC <- paste(stai$LIWC,stai$STAI)
stai <- stai[,-11]
...
tiff("Figures/SF_alltest2.tiff", units="cm", width=90, height=90, res=600)
...
tiff("Figures/SF_dend_new.tiff", units="cm", width=90, height=30, res=600)
...
png("Figures/SF_all_new.png", units="cm", width=100, height=100, res=600)
